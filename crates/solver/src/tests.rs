...
fn test_cnf_input() {
    ...
    let assumption_1 = Variable::from_index(0).unwrap().into_literal(Sign::POS);
    let assumption_2 = Variable::from_index(6).unwrap().into_literal(Sign::POS);
    let assumption_3 = Variable::from_index(5).unwrap().into_literal(Sign::POS);
